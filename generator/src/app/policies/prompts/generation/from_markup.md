...
Верни `Да`, если во входной разметке явно указано, что препарат отпускается по рецепту. Во всех остальных случаях верни `Нет`. Не используй формулировки `По рецепту`, `Не требуется`, `Нет данных.` или любые другие значения.
