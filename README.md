...
`.env` файлы не создаются автоматически. Если сервису нужны секреты, создайте `.env` вручную по соответствующему `.env.example`.

Основные секреты сейчас лежат в сервисных `.env`:
...
skills/gitlab-skill/.env:
GITLAB_BASE_URL=https://gitlab.com
GITLAB_TOKEN=...
```

Для нового сервиса используйте тот же паттерн: сервис читает свой `.env` через `env_file`, а секреты не коммитятся в репозиторий.
